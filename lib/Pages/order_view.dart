  ...
  void _addItemToOrder(ItemData item) async {
    ...
    try {
      ...
      if (existingOrderSnapshot.docs.isNotEmpty) {
        ...
        final existingItems = List<Map<String, dynamic>>.from(existingOrder.data()['items'] ?? []);
                          ...
                          return FutureBuilder(
                            future: Future.wait(
                              tables.map((table) async {
                                final tableData = Map<String, dynamic>.from(table.data() as Map<String, dynamic>);
                                ...
                                if (reservationSnapshot.docs.isNotEmpty) {
                                  ...
                                  tableData['customerName'] = reservationData['customerName'] ?? 'No name';
                                  tableData['currentGuests'] = reservationData['partySize'] ?? 0;
                                }
                                return {'id': table.id, 'data': tableData};
                              }).toList(),
                            ),
                            builder: (context, AsyncSnapshot<List<Map<String, dynamic>>> enhancedSnapshot) {
                              ...
                              return DropdownButtonFormField<String>(
                                ...
                                selectedItemBuilder: (BuildContext context) {
                                  return enhancedTables.map<Widget>((tableInfo) {
                                ...
                                items: enhancedTables.map<DropdownMenuItem<String>>((tableInfo) {
                                  final data = tableInfo['data'] as Map<String, dynamic>;
                                ...
                                onChanged: (String? value) {
                                  setState(() {
                                    ...
                                    if (value != null) {
                                      final selectedTableInfo = enhancedTables.firstWhere(
                                        (table) => table['id'] == value,
                                        orElse: () => enhancedTables.first,
                                      );
                                      selectedTable = selectedTableInfo['data'] as Map<String, dynamic>;
                                    }
                                  });
                                },
                              );
